...
Possibility DP. The same as one of the problem in Google Jam Code

To maximize the profit, when are faced with choice, choose the better one between previous best and now time profit
pos is the place where dp[i-1] should be fitted in
pos can use binary search to find.
presum restore the prefix sum in of gift value.
dp[k]=pos/N*dp[k-1]+(N-pos)/N*(presum[n]-presum[pos])/(N-pos)
